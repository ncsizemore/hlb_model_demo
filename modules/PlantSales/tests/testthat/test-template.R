...
#    `functionName` is a function's name in your module (e.g., `PlantSalesEvent1`).
# 2. Copy this file to the tests folder (i.e., `H:/Documents/florida-hlb-model/spades_model/modules/PlantSales/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("PlantSales")
  path <- list(modulePath = "H:/Documents/florida-hlb-model/spades_model/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    PlantSales = list(.saveInitialTime = NA)
  )
  ...
  if (exists("PlantSalesEvent1", envir = .GlobalEnv)) {
    simOutput <- PlantSalesEvent1(mySim)
  } else {
  ...
  if (exists("PlantSalesEvent2", envir = .GlobalEnv)) {
    simOutput <- PlantSalesEvent2(mySim)
  } else {
