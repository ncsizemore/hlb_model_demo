...
#    `functionName` is a function's name in your module (e.g., `CitrusInfectionEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/ncsizemore/Documents/florida-hlb-model/spades_model/modules/CitrusInfection/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("CitrusInfection")
  path <- list(modulePath = "C:/Users/ncsizemore/Documents/florida-hlb-model/spades_model/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    CitrusInfection = list(.saveInitialTime = NA)
  )
  ...
  if (exists("CitrusInfectionEvent1", envir = .GlobalEnv)) {
    simOutput <- CitrusInfectionEvent1(mySim)
  } else {
  ...
  if (exists("CitrusInfectionEvent2", envir = .GlobalEnv)) {
    simOutput <- CitrusInfectionEvent2(mySim)
  } else {
