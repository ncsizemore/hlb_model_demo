...
#    `functionName` is a function's name in your module (e.g., `PsyllidSpreadEvent1`).
# 2. Copy this file to the tests folder (i.e., `H:/Documents/florida-hlb-model/PsyllidSpread/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("PsyllidSpread")
  path <- list(modulePath = "H:/Documents/florida-hlb-model",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    PsyllidSpread = list(.saveInitialTime = NA)
  )
  ...
  if (exists("PsyllidSpreadEvent1", envir = .GlobalEnv)) {
    simOutput <- PsyllidSpreadEvent1(mySim)
  } else {
  ...
  if (exists("PsyllidSpreadEvent2", envir = .GlobalEnv)) {
    simOutput <- PsyllidSpreadEvent2(mySim)
  } else {
