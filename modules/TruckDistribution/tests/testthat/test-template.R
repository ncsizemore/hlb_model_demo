...
test_that("test Event1 and Event2.", {
  module <- list("TruckDistribution")
  path <- list(
    modulePath = "/Users/cristina/wiley/Documents/florida-hlb-model/spades_model/modules",
    outputPath = file.path(tempdir(), "outputs")
  )
  parameters <- list(
    #.progress = list(type = graphical, interval = 1),
    .globals = list(verbose = FALSE),
    TruckDistribution = list(.saveInitialTime = NA)
  )
