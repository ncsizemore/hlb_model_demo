...
#    `functionName` is a function's name in your module (e.g., `EggsEvent1`).
# 2. Copy this file to the tests folder (i.e., `H:/Documents/florida-hlb-model/spades_model/modules/Eggs/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  if (exists("EggsEvent1", envir = .GlobalEnv)) {
    simOutput <- EggsEvent1(mySim)
  } else {
  ...
  if (exists("EggsEvent2", envir = .GlobalEnv)) {
    simOutput <- EggsEvent2(mySim)
  } else {
